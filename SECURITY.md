...
- A command must be picked up within two minutes. The Mac records processed command IDs locally and acknowledges the command to the relay before execution, so a replayed, restored, or expired relay record cannot repeat or start an action. Once started, a command may run for up to ten minutes (the Computer Use limit), and its encrypted result is kept for six hours so a suspended phone can collect it later.
- Pairing does not itself authorize execution. Remote control starts paused for every new pairing, and the user must enable it on the Mac. That choice is saved in the Mac's local settings, so it persists across relaunches and updates until the user pauses it. It is restored only while the pairing is still active, and revoking or re-pairing the phone turns it off. The switch is never set by the phone or the cloud.
...
- Calls from Vox are disabled by default. When the owner enables them, Vox dials only the owner's encrypted callback number: for an immediate test call, or for a reminder the owner explicitly marked for phone-call delivery. A per-minute Cloudflare Cron Trigger places reminder calls; it reaches the dispatcher in-process with a token minted inside the running Worker, so outside requests to that route receive `404`. Each reminder is claimed atomically before dialing, retried at most three times on a provider error, and never called more than 30 minutes late. The call reads the reminder title and note aloud, so choose a callback number only you answer. Autonomous check-ins and third-party calls are not enabled.
