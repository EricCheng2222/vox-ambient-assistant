...
export function isDraftTransmissionTool(meta) {
  const toolName = meta?.tool_name;
  const toolParams = meta?.tool_params ?? {};
  const serialized = JSON.stringify(toolParams);
  if (toolName === "perform_secondary_action") return true;
  if (toolName === "press_key" && /(?:enter|return)/iu.test(serialized)) return true;
  if (["type_text", "paste", "set_value"].includes(toolName) && /\\[rn]/u.test(serialized)) {
    return true;
  }
  return toolName === "click" &&
    /(?:\bsend\b|\bsubmit\b|\bpost\b|\bpublish\b|\bshare\b|\bupload\b|傳送|送出|發送|寄出|發文|發布|分享|上傳)/iu.test(serialized);
}

export function permitsConfirmedApp(params, bundleId, threadId, options = {}) {
  ...
  return params?.threadId === threadId &&
    ...
    !(options.draftOnly && isDraftTransmissionTool(meta)) &&
...
export async function runComputerUseSession({ executable, cwd, prompt, bundleId, mode, signal, onItem, onDiagnostic = () => {}, completionCheck, completionAnswer = "", draftOnly = false }) {
  ...
  lines.on("line", (line) => {
    ...
    if (message.method && message.id !== undefined) {
      if (message.method === "mcpServer/elicitation/request") {
        send({ id: message.id, result: permitsConfirmedApp(message.params, bundleId, threadId, { draftOnly })
  ...
  try {
    ...
    await request("initialize", {
      clientInfo: { name: "vox_desktop", version: "0.3.0" },
